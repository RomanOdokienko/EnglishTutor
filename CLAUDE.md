...
- **An annotation finding dict is assembled in exactly ONE place: `build_finding` in cli.py** (it used to be three sites and fields silently vanished). A new field means: add it in `build_finding`, in the model schema, and — only if it should gate counting — in `is_countable_annotation`.
- **`severity` (blocking/noticeable/minor, ADR-0007) is on by default** since 2026-07-16; `OPENAI_ANNOTATION_SEVERITY=0` is the kill switch. It never affects the counting gate; empty = "not rated" (pre-rollout item), not "minor". Treat levels ordinally — blocking is legitimately rare/absent for these speakers.
- **Local builds strip the API base URL unless the env is set.** `write_web_assets` injects `ENGLISH_TUTOR_API_BASE_URL` into `out/web/*.html`; the committed artifacts carry the Railway URL. The key lives in the local git-ignored `.env` (cli.py reads it) — if HTML files show `""` in the diff, the env was missing. Don't commit that.
- **Timing metrics exist only where `sessions/<date>/timings.json` exists** (recorded sessions; ADR-0006). `refresh_timing_block` re-reads it inside `finalize_derived_metrics`, so `--recompute-derived` propagates threshold changes. Absent keys mean "not measured" — never render or average them as zero. `--backfill-timings` converts a stored raw AssemblyAI response.
